from condb2.ui.cli import CLI, CLICommand, InvalidOptions, InvalidArguments
from condb2 import ConDB, ConDBClient, CDFolder
from condb2.timelib import text2timestamp
...
def connect_to_db(opts, dbname):
    ...
    dbcon = []
    if "-h" in opts:
        dbcon.append("host=%s" % (opts["-h"],))
    if "-p" in opts:
        dbcon.append("port=%s" % (int(opts["-p"]),))
    if "-U" in opts:
        dbcon.append("user=%s" % (opts["-U"],))
    if "-w" in opts:
        dbcon.append("password=%s" % (opts["-w"],))
...
def read_csv(input_file):
    ...
    if tuple(columns[:2]) != ("channel","tv"):
        ...
        print("Headline from the file:", columns, file=sys.stderr)
        sys.exit(1)
    data = []
    for row in reader:
        if row:
